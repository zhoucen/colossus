...
//
//abstract  class WebSocketHandler extends Controller[BaseWebSocketMessage, BaseWebSocketMessage](new WebSocketServerCodec, ControllerConfig(50,10.seconds)) {
...
//
//
//  def processMessage(request: BaseWebSocketMessage) {
//    request match {
//      case request: HandshakeRequest =>
//        val headers = request.headers.foldLeft(Vector[HttpResponseHeader]()) { (headers, keyVal) =>
...
//          }
//        }
//        val response = HandshakeResponse(HttpResponse(HttpVersion.`1.1`, HttpCodes.SWITCHING_PROTOCOLS,  headers, ByteString("")))
//        push(response){_ => ()}
